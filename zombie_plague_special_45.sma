...
new g_isconnected[33], g_isalive[33], g_isbot[33], g_currentweapon[33], g_playerrank[33][32], g_playername[33][32], Float:g_spd[33], Float:g_custom_leap_cooldown[33], Float:g_zombie_knockback[33], g_pl_classname[33][64], g_pl_classname_lang[33];
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type) { // Ham Take Damage Forward
	...
	if(!is_user_valid_connected(attacker) && pev_valid(attacker)) {
		...
		if(damage >= fm_get_user_health(victim) && equal(classname, "trigger_hurt") && !g_escape_map) {
			if(g_endround)
...
public client_putinserver(id) { // Client joins the game
	...
	zp_get_user_rank(id, g_playerrank[id], charsmax(g_playerrank[]))
...
public show_menu_game(id) { // Game Menu
	...
	len += formatex(menu[len], charsmax(menu) - len, "%s %L^n", "\r4.\w", id, "KNIFEMENU")/* 4. it was Human class now its knife menu */ 
	...
	/* 6: XP */
	len += formatex(menu[len], charsmax(menu) - len, "%s %L^n", "\r6.\w", id, "XPMENU")
	len += formatex(menu[len], charsmax(menu) - len, "%s %L^n^n", "\r7.\w", id, "RZMENU")
...
public show_menu_buy(id, wpn_type) { // Buy Menu
	...
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\d%L \w[%d-%d]^n^n", id, (wpn_type == 0) ? "MENU_BUY1_TITLE" : "MENU_BUY2_TITLE", WPN_STARTID+1, min(WPN_STARTID+7, weapon_maxids))
	...
	for(weap = WPN_STARTID; weap < maxloops; weap++) {
		...
		else len += formatex(menu[len], charsmax(menu) - len, "\r%d.\y %s %s^n", weap-WPN_STARTID+1, szWeaponName, g_AdditionalMenuText)
	}
	len += formatex(menu[len], charsmax(menu) - len, "^n\r8.\y %L \r[%L]", id, "MENU_AUTOSELECT", id, (WPN_AUTO_ON) ? "MOTD_ENABLED" : "MOTD_DISABLED") /* 8. Auto Select */ 
	len += formatex(menu[len], charsmax(menu) - len, "^n^n\r9.\y %L/%L^n^n\r0.\w %L", id, "MENU_NEXT", id, "MENU_BACK", id, "MENU_EXIT") /* 9. Next/Back - 0. Exit */ 
...
show_menu_extras(id) { // Extra Items Menu
	...
	// Title
	formatex(menu, charsmax(menu), "%L [\d%s\y]\r", id, "MENU_EXTRA_TITLE", special_name)
	...
	for(item = 0; item < g_extraitem_i; item++) {
		...
		if(g_fwDummyResult >= ZP_PLUGIN_HANDLED || g_ammopacks[id] < ArrayGetCell(g_extraitem_cost, item)) {
			...
			formatex(menu, charsmax(menu), "%s \y%d \w%s", buffer, ArrayGetCell(g_extraitem_cost, item), g_AdditionalMenuText)
			buffer[0] = item
			buffer[1] = 0
...
public show_menu_zclass(id) { // Zombie Class Menu
	...
	for(class = 0; class < g_zclass_i; class++) { // Class List
		...
		if(g_fwDummyResult < ZP_PLUGIN_SUPERCEDE) { // Not Show the Zombie Class
			formatex(menu, charsmax(menu), "%s%s \d| \y%s \r%s %s", g_fwDummyResult >= ZP_PLUGIN_HANDLED ? "\d" : "\w", buffer, buffer2, (class == g_zombieclassnext[id]) ? "\dX" : "", g_AdditionalMenuText)
...
public menu_game(id, key) { // Game Menu
	switch (key) {
		...
		case 5: { // Human Classes
			client_cmd(id, "say /xp")
		}
		case 6: { // Human Classes
			client_cmd(id, "say /rinf")
		}
		case 8: { // Admin Menu
...
start_infection_mode(id, mode) { // Start the default infection mode
	...
	g_newround = false // No more a new round
	remove_task(TASK_MAKEZOMBIE) // Bug Fix
	ExecuteForward(g_forwards[ROUND_START], g_fwDummyResult, MODE_MULTI, forward_id); // Round start forward
	return 1
}
...
public welcome_msg() { // Welcome Message Task
	client_print_color(0, print_team_default, "^1**** ^4Zombie Escape %s^1 ****", VERSION) // Show mod info
...
public event_show_status(id) { // Some one aimed at someone
	if(!g_isbot[id] && g_isconnected[id] && get_pcvar_num(cvar_aiminfo)) { // Not a bot and is still connected
		...
		new aim_xp = zp_get_user_xp(aimid)
		new aim_maxxp = zp_get_user_max_xp(aimid)
		if(g_zombie[id] == g_zombie[aimid] || get_pcvar_num(cvar_aiminfo) == 2) {
			...
			ShowSyncHudMsg(id, g_MsgSync[2], "%L", id, "AIM_INFO", g_playername[aimid], class, fm_get_user_health(aimid), pev(aimid, pev_armorvalue), g_ammopacks[aimid], g_playerrank[aimid], aim_xp, aim_maxxp)
		}
		else {
			set_hudmessage(g_zombie[aimid] ? 255 : 0, 50, g_zombie[aimid] ? 0 : 255, -1.0, 0.60, 1, 0.01, 3.0, 0.01, 0.01, -1)
...
public ShowHUD(taskid) { // Show HUD Task
	...
	if(id != ID_SHOWHUD) { // Spectating someone else?
		...
		set_hudmessage(random_num(0,255), random_num(0,255), random_num(0,255), 0.01, 0.22, 1, 6.0, 1.1, 0.0, 0.0, -1)
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync[1], ">>[RINF Zombie Escape]^n>>[%L %s]^n>>[HP: %s]^n>>[%L %s]^n>>[%L %s]^n>>[%L: %s | %L: %s]^n>>[%L: %s]", ID_SHOWHUD, "SPECTATING", g_playername[id],
		add_point(fm_get_user_health(id)), ID_SHOWHUD, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", add_point(g_ammopacks[id]), ID_SHOWHUD, "XP", add_point(zp_get_user_xp(id)), ID_SHOWHUD, "Level", add_point(zp_get_user_level(id)), ID_SHOWHUD, "RANK", g_playerrank[id])
		return;
	}
	...
	static szHealth[64], szArmor[64], szAmmoPack[64], userDeaths, userFrags, userXp[64], usermaxXp[64], userLvl[64], userSpd;
	...
	userXp = add_point(zp_get_user_xp(id));
	usermaxXp = add_point(zp_get_user_max_xp(id)); 
	userLvl = add_point(zp_get_user_level(id));
	userSpd = fm_get_speed(id);
	
	switch(g_hud_type[id]) {
		case 0: { // Default
			set_hudmessage(random_num(0,255),random_num(0,255), random_num(0,255), 0.01, 0.22, 0, 6.0, 1.1, 0.0, 0.0, -1)
			ShowSyncHudMsg(id, g_MsgSync[1], ">>[RINF Zombie Escape]^n>>[%L: %s]^n>>[%L %s]^n>>[%L %s]^n>>[%L: (%s/%s)]^n>>[%L: %s | %L: %s%s", id, "ZOMBIE_ATTRIB1", szHealth, id, "CLASS_CLASS", 
			class, id, "AMMO_PACKS1", szAmmoPack, id, "XP", userXp, usermaxXp, id, "Level", userLvl, id, "RANK", g_playerrank[id], "]");
		}
		case 1: { // Classic
			set_hudmessage(0, 255, 255, -1.0, 0.85, 0, 6.0, 1.1, 0.0, 0.0, -1)
		...
		case 3: { // Antrax Style
			set_hudmessage(0, 255, 255, -1.0, 0.85, 1, 6.0, 1.1, 0.0, 0.0, -1)
...
public set_user_flashlight(taskid) { // Custom Flashlight
    ...
    if (get_distance_f(originF, destoriginF) > 35.0) return; // Max distance check
